...
def digit_distance(n):
    ...
    digit = n % 10
    n //= 10
    return abs(digit - n % 10) + digit_distance(n)
...
def count_coins(total):
    ...
    def helper(n, coin):
        if n == total:
            return 1
        elif n > total or coin == None:
            return 0
        else:
            this_coin = helper(n + coin, coin)
            next_coin = helper(n, next_larger_coin(coin))
