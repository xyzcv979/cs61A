    ...
    for i, word in enumerate(typed_words):
        if i >= len(source_words):
            ...
            if word == source_words[i]:
                correct_words += 1
    return correct_words / max_words * 100
    # END PROBLEM 3
...
def feline_fixes(typed, source, limit):
    ...
    if num_of_changes > limit:
        return limit + 1
    return num_of_changes + feline_fixes(typed[1:], source[1:], limit - num_of_changes)
    # END PROBLEM 6
    ...
    for i, word in enumerate(typed):
        if i >= source_words:
            ...
            if word == source[i]:
                ...
                break
    progress_ratio = progress / source_words
...
def time_per_word(words, timestamps_per_player):
    ...
    for player in timestamps_per_player:
        player_times = []
        for i, time in enumerate(player):
            if i == 0:
                continue
            else:
...
def fastest_words(match):
    ...
    output = [[] for i in range(len(all_times))]

    for word_idx, word in enumerate(all_words):
        fastest_time = 100000
        fastest_player = 0
        for i in range(len(all_times)):
            curr_time = all_times[i][word_idx]
            if curr_time < fastest_time:
                fastest_time = curr_time
                fastest_player = i
        output[fastest_player].append(word)
    return output
    # END PROBLEM 10
