...
def boar_brawl(player_score, opponent_score):
    ...
    players_ones_digit = player_score % 10
    outcome = 3 *  (abs(opp_tens_digit - players_ones_digit))
    return 1 if 1 > outcome else outcome
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score = 0
    opponent_score = 99
    while score < goal and opponent_score > 0:
        if strategy(score, opponent_score) != strategy(opponent_score, score):
            return False
        score += 1
        opponent_score -= 1
    return True
...
def make_averaged(original_function, samples_count=1000):
    ...
    def avg_func(*args):
        ...
        for i in range(samples_count):
            avg_values += original_function(*args)
        return avg_values / samples_count
    return avg_func
    # END PROBLEM 8
